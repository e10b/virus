...
std::vector<glm::ivec2> Terrain::generateTreePoints(glm::ivec2 startCorner, glm::ivec2 endCorner)
{
	...
	for (int z = startCorner.y; z < endCorner.y; z++)
	{
		for (int x = startCorner.x; x < endCorner.x; x++)
		{
			...
			// Only generate trees above sea level
			if (!isAboveSeaLevel(pos))
				continue;
...
std::vector<glm::ivec2> Terrain::generateFlowerPoints(glm::ivec2 startCorner, glm::ivec2 endCorner)
{
	std::vector<glm::ivec2> points;
	std::hash<glm::ivec2> hash;
	std::uniform_real_distribution<float> dist(0.0f, 1.0f);

	// check each point for if a flower should be generated
	for (int z = startCorner.y; z < endCorner.y; z++)
	{
		for (int x = startCorner.x; x < endCorner.x; x++)
		{
			...
			// Only generate flowers above sea level on grass
			if (!isAboveSeaLevel(pos))
				continue;

			// Use a different seed offset for flowers to avoid overlap with trees
			std::mt19937_64 rng(unsigned(hash(pos)) + 12345);

			// Generate flowers with lower density than trees (about 1/4 the density)
			if (dist(rng) <= Gen::treeDensity * 0.25f)
			{
				points.push_back(pos);
			}
		}
...
bool Terrain::isAboveSeaLevel(glm::ivec2 pos)
{
	return getHeight(pos) > Gen::seaLevel;
}