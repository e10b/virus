...
public:
	...
	Selection()
	{
		...
		vbo->setAttribute(1, wgfx::vec3f, 3); // normal
		vbo->setAttribute(2, wgfx::vec2f, 6); // uv
		vbo->setAttribute(3, wgfx::vec1f, 8); // texture index
	...
	void draw(const Camera& camera, wgfx::RenderPass pass, glm::ivec3 pos)
	{
		...
		// Get the block at the selection position to determine its type
		const Block& selectedBlock = chunks.getBlock(pos);
		
		// Don't draw selection for AIR blocks
		if (selectedBlock.type == Block::AIR) {
			return;
		}
		
		// Get the appropriate selection faces based on the block's collision AABB
		std::array<Face, Math::DIRECTION_COUNT> selectionFaces = BlockData::getSelectionFaces(selectedBlock.type);
		
		// Add all faces using the generated selection geometry
		for (int d = 0; d < Math::DIRECTION_COUNT; d++)
		{
			mesh.addQuad(selectionFaces[d], { 0,0 }, 0, pos, { 0,0,0 });
		}
		mesh.buffer(&mesh);
