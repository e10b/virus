...
public:
	...
	// get deterministic flower points in area [start, end)
	std::vector<glm::ivec2> generateFlowerPoints(glm::ivec2 startCorner, glm::ivec2 endCorner);

	// check if position is above sea level (for tree generation)
	bool isAboveSeaLevel(glm::ivec2 pos);
